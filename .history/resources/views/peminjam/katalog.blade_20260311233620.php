...
<div class="flex items-center justify-between mb-4 flex-wrap gap-3">
    ...
    <span class="text-muted text-sm">{{ $buku->total() }} buku</span>
</div>
...
<div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:14px;">
    ...
    <div class="card" style="transition:all .2s;overflow:visible;" onmouseenter="this.style.transform='translateY(-4px)';this.style.boxShadow='0 12px 40px rgba(99,102,241,.14)'" onmouseleave="this.style.transform='none';this.style.boxShadow=''">
        ...
        <div style="padding:14px;">
            <div class="font-sem truncate mb-1" style="font-size:13px;">{{ $b->judul }}</div>
...
<style>
    ...
    .pagination > * { padding: 6px 10px; border-radius: 6px; font-size: 13px; }
    .pagination .page-link { background: var(--card); border: 1px solid #e8ecf4; color: var(--text-2); text-decoration: none; transition: all 0.15s; }
    .pagination .page-link:hover { background: var(--brand-50); border-color: var(--brand-100); color: var(--brand-600); }
    .pagination .page-item.active .page-link { background: linear-gradient(135deg, var(--brand-500), var(--brand-700)); color: #fff; border-color: transparent; }
    .pagination .page-item.disabled .page-link { opacity: 0.5; cursor: not-allowed; background: #f1f5f9; }
    .pagination svg { width: 16px; height: 16px; }
</style>
<div class="mt-4 flex justify-end">{{ $buku->links() }}</div>
@endif
@endsection
