...
<div class="flex items-start justify-between gap-2 mb-1">
                ...
                <button type="button" onclick="toggleFavorit({{ $b->id }})" style="background:none;border:none;cursor:pointer;padding:4px;display:inline-flex;" title="{{ Auth::user()->isFavorit($b) ? 'Hapus dari favorit' : 'Tambah ke favorit' }}">
                    <i class="bi {{ Auth::user()->isFavorit($b) ? 'bi-heart-fill' : 'bi-heart' }}" style="color: {{ Auth::user()->isFavorit($b) ? '#ef4444' : '#94a3b8' }};font-size:18px;"></i>
                </button>
            </div>
